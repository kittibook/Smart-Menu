...
function createRouter(init) {
  ...
  function completeNavigation(location2, newState, { flushSync: flushSync2 } = {}) {
  ...
  async function startNavigation(historyAction, location2, opts) {
    ...
    if (matches && state.initialized && !isRevalidationRequired && isHashChangeOnly(state.location, location2) && !(opts && opts.submission && isMutationMethod(opts.submission.formMethod))) {
      completeNavigation(location2, { matches }, { flushSync: flushSync2 });
      return;
    }
  ...
  async function handleLoaders(request, location2, matches, scopedContext, isFogOfWar, overrideNavigation, submission, fetcherSubmission, replace2, initialHydration, flushSync2, pendingActionResult) {
  ...
  async function handleFetcherAction(key, routeId, path, requestMatches, scopedContext, isFogOfWar, flushSync2, preventScrollReset, submission) {
    interruptActiveLoads();
  ...
  async function handleFetcherLoader(key, routeId, path, matches, scopedContext, isFogOfWar, flushSync2, preventScrollReset, submission) {
    let existingFetcher = state.fetchers.get(key);
    updateFetcherState(
      key,
      getLoadingFetcher(
...
function RSCHydratedRouter({
  ...
  if (payload.type !== "render") throw new Error("Invalid payload type");
  let { router, routeModules } = React42.useMemo(
  ...
  let [location2, setLocation] = React42.useState(router.state.location);
  React42.useLayoutEffect(
